...
function get_cities_cloud()
{
	global $db;
	$city_array = array();
	$sql = 	'SELECT c.id, c.name, c.ascii_name, COUNT(*) AS nr FROM '.DB_PREFIX.'cities c 
	...
	foreach ($cities as $city)
	{
		...
		$city_array[] = array('name' => $city['name'],'varname' => $city['ascii_name'],
		'count' => $numberOfJobs,'tag_height' => get_cloud_tag_height($numberOfJobs));
	}
	return $city_array;
}
...
function get_categories()
{
    global $db;
    $categories = array();
    $sql = 'SELECT *FROM '.DB_PREFIX.'categories ORDER BY category_order ASC';
    $result = $db->query($sql);
...
function get_articles()
{
	global $db;
	$articles = array();
	$sql = 'SELECT id, title, page_title, url FROM '.DB_PREFIX.'pages ORDER BY title ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_navigation($menu = false)
{
	...
	$navigation = array();
	$sql = 'SELECT id, url, name, title, menu FROM '.DB_PREFIX.'links' . $conditions . '
...
function get_cities()
{
	global $db;
	$cities = array();
	$sql = 'SELECT DISTINCT id, name , ascii_name,state_id
...
function get_cities_list($state_id)
{
    ...
    $cities = array();
    $sql = 'SELECT DISTINCT id, name , ascii_name,state_id
                   FROM '.DB_PREFIX.'cities WHERE state_id = '.$state_id.'
                   ORDER BY name ASC';
    $result = $db->query($sql);
    while ($row = $result->fetch_assoc())
    {
...
function get_states()
{
	global $db;
	$states = array();
	$sql = 'SELECT * FROM '.DB_PREFIX.'states
	 ORDER BY name ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
		$states[] = array('id' => $row['id'],'name' => $row['name']);
	}
	return $states;
}
function get_others()
{
    ...
    $sql = 'SELECT DISTINCT id,category,dependent,others,user_id
                   FROM '.DB_PREFIX.'others
                   ORDER BY id ASC';
    ...
    while ($row = $result->fetch_assoc())
    {
        $others[] = array('id' => $row['id'], 'category' => $row['category'], 'dependent' => $row['dependent'], 'others' => $row['others'], 'user_id' => $row['user_id']);
    }
   
    return $others;
}
function get_others_values()
{
	if($_SESSION['UserId'] == '')
	{
		$id = 0;
	...
	global $db;
	$others = array();
	$sql = 'SELECT id, category, dependent, others, user_id FROM '.DB_PREFIX.'others where user_id = '.$id.' ORDER BY id ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
		$others[] = array('id' => $row['id'], 'category' => $row['category'],'dependent' => $row['dependent'], 'others' => $row['others'] , 'user_id' => $row['user_id']);
...
function jobs_list() 
{
	global $db;
	$jobslist = array();
	$sql = 'SELECT * FROM '.DB_PREFIX.' types_jobs ORDER BY name ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_years()
{
	global $db;
	$years = array();
	$sql = 'SELECT * FROM '.DB_PREFIX.'years ORDER BY id ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_months()
{
	global $db;
	$months = array();
	$sql = 'SELECT *   FROM '.DB_PREFIX.'months ORDER BY id ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_salary()
{
	global $db;
	$salary = array();
	$sql = 'SELECT * FROM '.DB_PREFIX.'salary ORDER BY id ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_salary_thousands()
{
	global $db;
	$salary = array();
	$sql = 'SELECT * FROM '.DB_PREFIX.'salary_thousands ORDER BY id ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_qualification()
{
	global $db;
	$qualification = array();
	$sql = 'SELECT DISTINCT id,qualification  FROM '.DB_PREFIX.'qualification 
	  ORDER BY id ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_industries()
{
	global $db;
	$industries = array();
	$sql = 'SELECT DISTINCT industry_id, name	FROM '.DB_PREFIX.'industries
	               ORDER BY name ASC';
	$result = $db->query($sql);
	while ($row = $result->fetch_assoc())
	{
...
function get_user_info()
{
	...
	$userid=$_SESSION['UserId'] ;
	 $sql = 'SELECT email,firstname,lastname,contact_number,sms,city_id,state_id,gender,category,total_exp,salary,
...
function get_year_of_passout()
{
 global $db;
 $cities = array();
 $sql = 'SELECT id,year_of_passout FROM year_of_passout ORDER BY id  DESC';
 $result = $db->query($sql);
 while ($row = $result->fetch_assoc())
 {
...
function get_user()
{
	global $db;
		$user = array();
	$userid=$_SESSION['UserId'] ;
  $sql='select firstname from user_info where id="'.$userid.'"'; 
	$result = $db->query($sql);
		while ($row = $result->fetch_assoc())
	{
			$use[] = array(
...
//function to display count the number of jobs applyed
function jobsapplyedcount()
{
	global $db;
		$apply = array();
	$userid=$_SESSION['UserId'] ;
  $sql='SELECT COUNT( a.job_id ) AS totalNumberOfApplications FROM job_applications a WHERE  `user_id`="'.$userid.'"'; 
	$result = $db->query($sql);
	$row = $result->fetch_assoc();
 ...
 function get_user_skill()
{
	global $db;
	$userid=$_SESSION['UserId'] ;
	$sql = 'SELECT skills FROM user_info WHERE id="'.$userid.'"';
	$result = $db->query($sql);
	$row = $result->fetch_assoc();
	$skills=$row['skills'];
	return $skills;
