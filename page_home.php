<?php
    ...
    $smarty->assign('use', get_user());
    $smarty->assign('totalNumberOfApplications', jobsapplyedcount());
    $skill = get_user_skill();
	$skill = explode(',',$skill);
    $skills_name=$skill[0];
	$smarty->assign('user', get_user_info());
	$smarty->assign('skillsasperjobs', $job->skilljobs($skills_name));
	$template = 'index.tpl';
?>